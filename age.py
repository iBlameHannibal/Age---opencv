...
# List of age and gender classes
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.7:  # Adjust confidence threshold if needed
            ...
            if face.shape[0] > 0 and face.shape[1] > 0:
                ...
                # Prepare the face ROI for age classification
                age_net.setInput(blob)
                age_preds = age_net.forward()
                age = AGE_LIST[age_preds[0].argmax()]

                # Draw the bounding box and the predicted gender and age
                label = f"{gender}, {age}: {confidence * 100:.2f}%"
